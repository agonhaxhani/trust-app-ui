...
export class ConfigsService {
  ...
  saveConfigsToStorage(data: any) {
    const dataToSave = JSON.stringify(data);
    localStorage.setItem(GeneralConstant.LOCALSTORAGE_CONFIGS, dataToSave);
  }
  ...
  getConfigFromLocalStorage() {
    const configs = localStorage.getItem(GeneralConstant.LOCALSTORAGE_CONFIGS);
    if (!configs) {
